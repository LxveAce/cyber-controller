...
- **2026-07-01 ~12:20 — v1.5.0 RELEASE completed.** A cadence check caught that `build-release.yml` triggers on **`release: published`**, NOT on a tag push — so `git push origin v1.5.0` had created a dead tag with no build. Published the **GitHub Release for v1.5.0** (notes from the CHANGELOG) → the Build & Release workflow is now **queued/building** the Windows/macOS/Linux binaries + Inno installer (run 28517074060). Verifying it goes green next cycle. (Unsigned build — code-signing still owner-gated.)

- **2026-07-01 ~12:38 — v1.5.0 macOS build fixed.** The release build failed ONLY on macOS: the `macos-latest` runner image stopped preinstalling Pillow, so PyInstaller couldn't convert `assets/icon.ico`→`.icns` (`ModuleNotFoundError: PIL`). Windows/Linux/Linux-ARM shipped fine. Fixed `build-release.yml` (`0b4025d`): add `pip install pillow` to the macOS job + made it **continue-on-error** and **workflow_dispatch-capable** (mirrors the ARM job), so a macOS hiccup can never fail a whole release again AND macOS can be re-attached to an existing tag without moving it. Dispatched the macOS build for v1.5.0 (run 28518083197) → attaches the macOS binary + sha to the release. Verifying next cycle. **v1.5.0 = Windows + Linux + Linux-ARM shipped; macOS attaching.**

- **2026-07-01 ~12:49 — Windows INSTALLER fixed (never shipped before).** Found the Inno installer step invoked ISCC via `"$Env:ProgramFiles(x86)\..."` — PowerShell parses `(x86)` as a call and yields the space-less `C:\Program Files(x86)\...` (nonexistent), so ISCC was never found and `installer/Output/*.exe` matched nothing; `continue-on-error` masked it → **no release ever actually shipped the installer.** Fixed (`7e96bad`): brace-wrap `${Env:ProgramFiles(x86)}` + made build-windows workflow_dispatch-able (env.TAG + `tag_name` on uploads, mirroring macOS/ARM). Dispatched the re-attach for v1.5.0 (run 28518708435) → Windows core + **installer**, macOS, ARM all attach. Prior macOS/ARM dispatch 28518083197 = SUCCESS (macOS binary confirmed on the release). Verifying the installer next cycle.

### v1.5.0 COMPLETE — all 4 platforms + working installer (2026-07-01 ~12:59)

The v1.5.0 GitHub Release now carries **9 assets**, all verified present: Windows core `.exe` + **`windows-x64-setup.exe` (Inno installer — shipped for the FIRST time; the ISCC `ProgramFiles(x86)` path bug had silently broken it in every prior release)**, Linux-x64, Linux-ARM64, macOS-ARM64, each with a `.sha256`. Both re-attach dispatch runs green. Release-pipeline hardening this cycle: release-event vs tag-push understood + Release published; macOS Pillow `.ico→.icns` fix + best-effort + dispatch-able; Windows installer path fixed + dispatch-able. **v1.5.0 is genuinely, fully shipped.** Loop → measured hourly cadence; all remaining work is owner-gated (see Owner Decisions).
