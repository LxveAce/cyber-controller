...
- **Source-only firmwares** (no release binary the engine can fetch): minigotchi-v3 (release 404), airtag-scanner, sky-spy, flock-you, oui-spy. These require a local Arduino/PlatformIO build before they can be flashed. **`cyt-ng` is NOT in this class**: upstream `ArgeliusLabs/Chasing-Your-Tail-NG` is a Linux/Pi **Python** probe-request analyzer with no ESP32/Arduino/PlatformIO project — there is nothing to flash at all (see its profile note; owner decision pending per BLOCKED-BY-OWNER).
