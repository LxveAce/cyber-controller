...
**Cyber Controller** is a security-research tool, provided for **authorized, lawful use only**.
...
## High-risk features
This app can drive firmware that transmits RF and runs deauth/attack flows. For a connected **jammer**, the app **ships no attack or control frames**: the per-mode control payloads are empty and unvalidated by default, so out of the box the app cannot arm the device. A **STOP (Idle) action is always available and is never gated**. Any *arm* path additionally requires a control map that **you** capture and validate on your own hardware — **none ships** — plus an explicit RF-shielded-enclosure confirmation before a jamming mode will engage. Operating an RF jammer is illegal in most jurisdictions (47 U.S.C. §333 + worldwide equivalents) — use this **only inside an authorized RF-shielded enclosure, on hardware you own**. Deauth, evil-portal, and beacon/BLE-spam flows are illegal against networks you don't own or aren't authorized to test.
