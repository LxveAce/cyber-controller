...
class BruceProtocol(BaseProtocol):
    ...
    def get_commands(self) -> list[CommandInfo]:
        ...
        return [
            ...
            # IRSend->sendDecodedCommand — all transmit). No keyword → explicit gate.
            CommandInfo("ir tx_raw <freq> <samples>", "Offensive", "Transmit raw IR",
                        "freq samples", danger="lab-only"),
            CommandInfo("ir tx_from_buffer", "Offensive", "Transmit an .ir streamed over serial",
                        danger="lab-only"),
            CommandInfo("IRSend <json>", "Offensive", "Transmit an IR code (Tasmota JSON)",
                        "json", danger="lab-only"),
            # ---- SubGHz ----
            ...
            CommandInfo("subghz txp", "Offensive", "TX a SubGHz signal by protocol name",
                        "proto freq bits key te repeat", danger="lab-only"),
            CommandInfo("subghz keeloqtx", "Offensive", "TX a KeeLoq rolling-code frame",
                        "mfr freq btn serial counter repeat", danger="lab-only"),
            CommandInfo("subghz tx_from_buffer", "Offensive", "TX a .sub streamed over serial",
                        danger="lab-only"),
            CommandInfo("RfSend <json>", "Offensive", "TX a raw RF code (Tasmota JSON)",
                        "json", danger="lab-only"),
            # KeeLoq keystore: add ARMS keeloqtx (attack-config, lab-only); list/clear = SAFE reads.
            CommandInfo("subghz mfcodes add <entry>", "Offensive", "Add a KeeLoq key (arms tx)",
