"""Tests for the wardrive KML/GPX exporters (WS6) — src/core/wardrive.py.

Covers: one map feature per network, well-formed XML (parsed with defusedxml), the XML-escaping of
attacker-controlled SSID/BSSID (the XML analog of the CSV-injection export rule), the empty-input
case, and the ``--wardrive-kml`` / ``--wardrive-gpx`` CLI wrapper.
"""
from __future__ import annotations

import defusedxml.ElementTree as ET

from src.core.wardrive import wardrive_export_cli, wigle_csv_to_gpx, wigle_csv_to_kml
...
def test_export_cli_unknown_format_returns_1(tmp_path, capsys):
    ...
    assert wardrive_export_cli(str(p), "geojson") == 1
