"""Flock heatmap (FL F4) — the offscreen heatmap widget rendered into a QImage and asserted on.

The pure web-mercator projection/geometry unit tests moved to tests/test_flock_heatmap_geometry.py (which
the hosted runner collects). This file keeps the Qt widget tests; it uses the ``qapp`` fixture, so the
hosted runner excludes it. Fed by F2's FlockSession GeoJSON.
"""
...
from src.ui.qt.flock_heatmap_tab import MercatorFit, world_px, zoom_step
