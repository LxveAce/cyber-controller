...
**[v1.8.0](https://github.com/LxveAce/cyber-controller/releases/latest)** — **LxveOS is now a first-class firmware**: recognize, flash, and control LxveOS boards end-to-end over the `LXVEOS/1` serial protocol, validated on real ESP32 silicon. A new **Operate console** gives you a single-device, button-driven control surface with a SAFE/ARMED lamp and two-factor arm — offensive transmit verbs stay locked until the device is ARMED. And **passive network-intel** lands on the Targets surface: OUI→vendor labels, a channel-occupancy survey, and target-freshness, all passive (also on the web remote via `/api/channels` and `/api/freshness`). Rounding it out: a firmware-variant (board) picker on the web flash page, CYD board-detection that actually applies its detected panel, a reachable JSON export in the Crack Lab (CSV or JSON), and a broad security + reliability hardening sweep. Backend/serial control only — CC issues firmware CLI commands and never authors radio frames.
