...
[![Firmwares](https://img.shields.io/badge/firmware%20profiles-33-success?style=for-the-badge)](#supported-firmwares)
...
**Status:** **v1.6.8** is the latest release — a **LxveLabs** project by LxveAce. It follows the three
...
+ autodetect fixes. It sits on 1.5.x's security-hardening + reliability passes (33 firmware profiles across five
...
### Flash
- **33 firmware profiles** across **5 backends**: `esptool` (ESP32 family), `qFlipper` (Flipper Zero),
...
33 firmware profiles ship in `src/config/profiles/`. Each tracks its **latest upstream release** at
...
| **WiFiDuck** (Wi-Fi BadUSB) ⚠ *authorized testing only* | [SpacehuhnTech/WiFiDuck](https://github.com/SpacehuhnTech/WiFiDuck) | **ESP8266** half (DSTIKE WiFi Duck / Malduino W) | esptool (merged) |
| **ESP32 WiFi Penetration Tool** ⚠ *authorized testing only* | [risinek/esp32-wifi-penetration-tool](https://github.com/risinek/esp32-wifi-penetration-tool) | ESP32 (DevKit / WROOM) | esptool (SHA-256-pinned) |
...
| **Flipper RogueMaster** | [RogueMaster/flipperzero-firmware-wPlugins](https://github.com/RogueMaster/flipperzero-firmware-wPlugins) | STM32WB55 | qFlipper |
